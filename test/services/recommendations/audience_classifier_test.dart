...
void main() {
  group('inferAudience', () {
    test('kids markers — case-insensitive word boundaries', () {
      expect(inferAudience(_p('Kids Multivitamin Gummies')), Audience.kids);
      expect(inferAudience(_p("Children's Daily Multi")), Audience.kids);
      expect(inferAudience(_p('Junior Probiotic Chewable')), Audience.kids);
      expect(inferAudience(_p('Toddler Vitamin D')), Audience.kids);
    ...
    test('prenatal markers — case-insensitive', () {
      ...
      expect(inferAudience(_p('Pregnancy Support Formula')), Audience.prenatal);
      expect(inferAudience(_p('Fertility Boost for TTC')), Audience.prenatal);
    ...
    test('sport markers (checked before mens — performance overrides)', () {
      ...
      // ordering.
      expect(
        inferAudience(_p("Men's Performance Pre-Workout")),
        Audience.sport,
      );
      expect(inferAudience(_p('Athletic Endurance Blend')), Audience.sport);
      expect(inferAudience(_p('Pre-Workout Cherry Limeade')), Audience.sport);
      expect(inferAudience(_p('Muscle Builder Stack')), Audience.sport);
    });

    test('mens / womens markers', () {
      expect(inferAudience(_p("Women's Daily Multi")), Audience.womens);
      expect(
        inferAudience(_p("Men's Multi 50+")),
        Audience.senior,
      ); // senior wins
  ...
  group('isAudienceCompatible — hard walls', () {
    test('kids is a complete wall — only kids ↔ kids', () {
      ...
      expect(isAudienceCompatible(Audience.kids, Audience.general), isFalse);
      expect(isAudienceCompatible(Audience.kids, Audience.prenatal), isFalse);
      expect(isAudienceCompatible(Audience.kids, Audience.mens), isFalse);
      expect(isAudienceCompatible(Audience.general, Audience.kids), isFalse);
    });

    test('prenatal is a complete wall — only prenatal ↔ prenatal', () {
      expect(
        isAudienceCompatible(Audience.prenatal, Audience.prenatal),
        isTrue,
      );
      expect(
        isAudienceCompatible(Audience.prenatal, Audience.general),
        isFalse,
      );
      expect(isAudienceCompatible(Audience.prenatal, Audience.womens), isFalse);
      expect(isAudienceCompatible(Audience.womens, Audience.prenatal), isFalse);
    });

    test('mens ↔ womens / senior cross-blocks', () {
      expect(isAudienceCompatible(Audience.mens, Audience.womens), isFalse);
      expect(isAudienceCompatible(Audience.mens, Audience.senior), isFalse);
      expect(isAudienceCompatible(Audience.womens, Audience.senior), isFalse);
      // Same specific audience swaps allowed.
      expect(isAudienceCompatible(Audience.mens, Audience.mens), isTrue);
      expect(isAudienceCompatible(Audience.senior, Audience.senior), isTrue);
    });

    test('general compatible with mens/womens/senior/sport', () {
      expect(isAudienceCompatible(Audience.general, Audience.mens), isTrue);
      expect(isAudienceCompatible(Audience.general, Audience.womens), isTrue);
      expect(isAudienceCompatible(Audience.general, Audience.senior), isTrue);
      expect(isAudienceCompatible(Audience.general, Audience.sport), isTrue);
    });
    ...
    test('sport ↔ general allowed at this layer — ranker carves out '
      ...
      expect(isAudienceCompatible(Audience.sport, Audience.general), isTrue);
      expect(isAudienceCompatible(Audience.general, Audience.sport), isTrue);
      expect(isAudienceCompatible(Audience.sport, Audience.sport), isTrue);
    });
  });
