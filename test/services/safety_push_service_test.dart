...
void main() {
  ...
  group('acquireAndRegisterToken', () {
    test('iOS asks permission, waits for APNs, then requests the FCM token',
        () async {
      debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
      final messaging = _FakeMessaging(apnsTokenAfterPolls: 1, fcmToken: 't1');
      ...
      expect(
        messaging.log,
        ['requestPermission', 'getAPNSToken', 'getAPNSToken', 'getToken'],
      );
    });

    test('iOS never calls getToken while the APNs token stays missing',
        () async {
      debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
      final messaging = _FakeMessaging(apnsTokenAfterPolls: 99, fcmToken: 't1');
      final service = SafetyPushService(
        messaging: messaging,
        invokeFunction: (_, __) async {},
      );
      ...
      expect(messaging.log, isNot(contains('getToken')),
          reason: 'getToken throws without an APNs token; registration must '
  ...
  group('unregisterBeforeSignOut', () {
    test('removes the server row, then invalidates the local token', () async {
      ...
      expect(
        calls.single.$2,
        {'action': 'unregister', 'token': 'live-token'},
      );
    ...
    test('still invalidates the local token when the server call fails',
        () async {
      final messaging = _FakeMessaging(fcmToken: 'live-token');
      final service = SafetyPushService(
        messaging: messaging,
    ...
    test('with no readable token it skips the server call and still deletes',
        () async {
      final messaging = _FakeMessaging(getTokenThrows: true);
      final calls = <String>[];
      final service = SafetyPushService(
