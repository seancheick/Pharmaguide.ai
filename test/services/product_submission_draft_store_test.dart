...
void main() {
  ...
  test(
    'a saved capture survives a restart with identical bytes and ids',
    () async {
      await _save(store);
      ...
      expect(
        restored.photos.single.categories,
  ...
  test(
    'restoring rebuilds the same submission so a retry is idempotent',
    () async {
      await _save(store);
      ...
      expect(
        first.photos.single.contentSha256,
        second.photos.single.contentSha256,
      );
    },
  );

  test(
    'discarding removes the label images from disk, not just the record',
    () async {
      await _save(store);
      ...
      expect(
        ...
        isEmpty,
        reason:
            'private label photos must not outlive the draft that owns them',
      );
    },
  );

  test(
    'a truncated manifest is discarded instead of crashing capture',
    () async {
      await _save(store);
      final manifest = root
          .listSync(recursive: true)
  ...
  test(
    'a photo whose bytes changed on disk is not restored as evidence',
    () async {
      await _save(store);
      final image = root
          .listSync(recursive: true)
      ...
      expect(
        ...
        isNull,
        reason:
            'the manifest hash is the evidence contract; altered bytes are not it',
      );
    },
  );

  test(
    'a manifest cannot claim a different submission than its directory',
    () async {
      await _save(store);
      final manifest = root
          .listSync(recursive: true)
      ...
      decoded['submission_id'] = _photoIdB;
      await manifest.writeAsString(jsonEncode(decoded), flush: true);

      expect(
        await store.restore(_userA, _submissionId),
        isNull,
        reason: 'directory and manifest identity must be one immutable record',
      );
      expect(
        await store.list(_userA),
        isEmpty,
        reason: 'a tampered identity must not be offered for resume',
      );
    },
  );

  test('a malformed category entry is rejected during restore', () async {
    await _save(store);
    final manifest = root
        .listSync(recursive: true)
        .whereType<File>()
    ...
    final decoded =
        jsonDecode(await manifest.readAsString()) as Map<String, dynamic>;
    final photos = (decoded['photos'] as List).cast<Map<String, dynamic>>();
    photos.single['categories'] = [42];
    ...
    expect(
      await store.restore(_userA, _submissionId),
      isNull,
      reason: 'corrupt category data must not escape the restore boundary',
    );
  });
  ...
  test(
    'the newest capture for a barcode is the one offered for resume',
    () async {
      await _save(store);
  ...
  test('drafts are kept under app-private support storage', () async {
    ...
    final source = File(
      'lib/services/product_submission_draft_store.dart',
    ).readAsStringSync();
  ...
  test(
    'an unreadable draft directory reports empty instead of throwing',
    () async {
  ...
  group('accounts are isolated', () {
    test(
      'another account cannot see, resume or discard these photos',
      () async {
        await _save(store);
    ...
    test('a capture cannot be stored without an account', () async {
      expect(() => _save(store, userId: ''), throwsA(isA<ArgumentError>()));
      expect(
      ...
      expect(
        () => _save(store, submissionId: '../../escape'),
        throwsA(isA<ProductSubmissionValidationException>()),
        reason: 'a submission id must not escape the account directory',
      );
    });
  });
  ...
  group('recovery survives interruption', () {
    ...
    test(
      'a write interrupted before the manifest leaves nothing to resume',
      () async {
        await _save(store);
        final manifest = root
            .listSync(recursive: true)
    ...
    test(
      'a photo missing from disk fails the whole capture, not silently',
      () async {
        await _save(
          store,
          photos: [
            _photo(seed: 1),
            _photo(id: _photoIdB, seed: 2),
          ],
        );
        final image = root
            .listSync(recursive: true)
    ...
    test(
      'concurrent saves of the same capture leave one consistent record',
      () async {
        await Future.wait([
