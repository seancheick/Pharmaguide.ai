    ...
    MaterialApp(
      home: Scaffold(
        body: SingleChildScrollView(
          child: SynergyDetailSection(synergyDetail: {'clusters': clusters}),
        ),
      ),
    ),
  );
...
void main() {
  ...
  group('SynergyDetailSection — render gating (T22)', () {
    testWidgets('0 clusters → entire section hidden', (tester) async {
      await _pump(tester, clusters: const []);
      await tester.pumpAndSettle();
      expect(find.text('Works well with'), findsNothing);
    });

    testWidgets('0 high-confidence clusters → entire section hidden '
        '(despite raw clusters present)', (tester) async {
    ...
    testWidgets('2 passing clusters → renders both as chips', (tester) async {
      await _pump(
        tester,
        clusters: [
    ...
    testWidgets('5 passing clusters → cap at 3 (top 3 by tier+match_count)', (
      tester,
    ) async {
      await _pump(
        tester,
  ...
  group('SynergyDetailSection — chip tap → modal (preserves T6)', () {
    testWidgets('tap chip opens bottom sheet with explanation + study count', (
      tester,
    ) async {
      await _pump(
        tester,
