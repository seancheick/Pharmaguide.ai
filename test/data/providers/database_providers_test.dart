...
void main() {
  ...
  group('openCoreDatabase rollback safety', () {
    test(
      ...
      () async {
        ...
        expect(
          productCount,
          greaterThan(0),
          reason: 'happy-path open returned a DB with no products — the '
    ...
    test(
      'corruption fallback: byte-length-matched garbage on disk → restore '
      'from bundle, retry open, return a working DB',
      () async {
        ...
        final assetData =
            await rootBundle.load(bundledCoreDatabaseAssetPath);
        ...
        expect(
          productCount,
          greaterThan(0),
          reason: 'rollback path returned a DB but countProducts came '
        ...
        expect(
          await dbFile.length(),
          assetBytes.length,
          reason: 'after rollback, the on-disk file size should match '
