-- Canonical identity is a comparison key only. Keep every original barcode,
-- manifest, formula detail, and lineage value immutable on create replay.
CREATE FUNCTION public.product_submission_canonical_gtin(p_normalized_upc text)
RETURNS text
LANGUAGE sql
IMMUTABLE
STRICT
SET search_path = ''
AS $$
  SELECT lpad(p_normalized_upc, 14, '0');
$$;

REVOKE ALL ON FUNCTION public.product_submission_canonical_gtin(text)
...
-- Use the same identity for the human duplicate check, approval lock, and
-- unpromoted approval uniqueness. Review payloads and authority are unchanged.
...
BEGIN
  ...
  IF p_to_status = 'approved' THEN
    ...
    ELSE
      review_target_key := 'missing_product:'
        || public.product_submission_canonical_gtin(submission.normalized_upc);
    END IF;
        ...
        AND (
          CASE other.kind
            ...
            ELSE 'missing_product:'
              || public.product_submission_canonical_gtin(other.normalized_upc)
          END
        ) = review_target_key
    ) THEN
...
-- Match the existing canonical unique index without changing stored digits.
...
BEGIN
  IF NEW.normalized_upc IS NOT NULL AND EXISTS (
    ...
    WHERE existing.id <> NEW.id
      ...
      AND public.product_submission_canonical_gtin(existing.normalized_upc)
        = public.product_submission_canonical_gtin(NEW.normalized_upc)
...
-- Advisory intake: a read-only owner-scoped receipt lookup. The final create
-- and finalize transactions remain responsible for races and replay safety.
CREATE FUNCTION public.get_product_submission_intake(
  p_kind public.product_submission_kind,
  p_upc text,
...
REVOKE ALL ON FUNCTION public.get_product_submission_intake(
  public.product_submission_kind, text, text
) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_product_submission_intake(
  public.product_submission_kind, text, text
) TO authenticated;
