...
- Age bracket → signal mapping: `under_*` / contains "child" → `under_18`; contains "65" / "over" → `over_65`. Empty / unrecognized → empty signal, ignored.
...
- **NEW** `test/features/product_detail/widgets/populations_section_test.dart` — 20 tests across 3 groups: pure `splitPopulations` (13 tests covering empty input, no signals, single-condition match, keyword match via "diabetic" → "diabetes", word-boundary defense, case-insensitivity, multi-population-same-signal collapse, all-match → empty main, drug-class matching documented, age-bracket flow, dedupe of duplicate strings, blank drop, alphabetical sort) + `aggregatePopulations` (1 test) + widget render (5 tests covering hide-when-empty, main-only, mixed main+covered, all-covered, age-bracket integration).
