...
**Verification (post Option-C refactor, 2026-04-29):** `flutter analyze` → No issues found · `flutter test` → **975/975 tests pass** · **34 apple-grade commits + cross-team merges** with Trust/IA on `origin/main`.

**Sprint 27.21 status: ✅ CLOSED on apple-grade side.** Phase G follow-ups: G.1, G.2, G.3 ✅ shipped; G.4 ⏸ deferred (design call); G.5 🚫 backlog. Phase H product-philosophy refactor: H.1, H.2 ✅ shipped (kill the combined number, kill dead code). All remaining holds are external (Trust/IA T-tasks or pipeline-gated).
