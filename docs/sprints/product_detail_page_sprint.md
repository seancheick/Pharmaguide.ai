...
- [x] **T7C** — A1/A2/A5 scoring refactor · `enrich_supplements_v3.py` + `score_supplements.py`
  ...
  - Verification (audited 2026-05-05, already shipped via v3.6.0): A1 — `score_supplements.py:837` reads `bio_score`; `:869-877` rescales `(avg_bio_score / 15) * 18`. Config `section_A_ingredient_quality.A1_bioavailability_form` has `range_score_field: "0-15"`, `score_field: "bio_score"`, `max: 18`. A2 — `:890` `threshold_score: 12.0`; `:913` reads `bio_score`; config `_max_rationale` documents the 14→12 threshold drop (12 on bio_score = 80% = Flutter Excellent tier). A5 — `:1063-1090` natural-source bonus moved from A1; config `A5_formulation_excellence.natural_source: 1`. Deprecated `score` alias documented inline (`:830-839, 909-913`); enricher emits `score == bio_score` so the fallback yields the same result. Shadow-compare tooling at `scripts/shadow_score_comparison.py` + `tests/test_shadow_score_comparison.py`. Devil's Claw test (`test_parent_fallback_final_state_guard.py:99`) already expects `score == bio_score` per v3.6.0 contract — confirms the alias semantics. Snapshot fixtures refreshed 2026-05-05 (manifest entry).
