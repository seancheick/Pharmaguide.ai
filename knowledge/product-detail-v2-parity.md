...
| **Status** | **verified** — live side-by-side vs production `/product/65844` (Thorne MediPro Vegan All-In-One, SAFE): title "Review before use" matches; info tone matches; auto-collapse behavior preserved; section correctly suppressed on blocked products (dsldId 1000). **V2 PRIVACY CORRECTION — Sean 2026-05-15 (replaces earlier "visible context line" approval):** the v2 card NEVER renders a generic always-visible "Your conditions: …" body line. Surfacing raw matched conditions just to prove profile matching happened leaks private health context when the user shows the screen to someone else and adds no clinical value by itself. New rule: condition names appear ONLY inside a specific warning row's caption (e.g. "Relevant profile factor: Diabetes") and only when that condition is the reason the warning fires. If no actionable warning/allergen/free-from signal exists, the card is suppressed entirely (does NOT render context-only). Affirmative-only mode (free-from claim with no warnings) still renders with its existing tone + body — context is intentionally excluded from the affirmative-tone trigger so a profile match alone never flips the card to "Allergen check passed". Helper ports remain verbatim: `_parseHint` / `_computeTone` / `_allergenTone` / `_severityColor` / `_countCopy` / `_affirmativeCopy` / `_humanLabel` / `_humanConcern`. Row-caption surfacing of profile factors is queued as **S3.next-iteration**. Other paths (warnings populated, allergen-match danger, nudge-banner) statically parity-checked via verbatim ports + exhaustive sealed-class switch. |
