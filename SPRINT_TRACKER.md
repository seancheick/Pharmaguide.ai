...
**Sprint 27.21: App-wide Apple-grade polish (in flight)** — 🟡 PARTIAL (2026-04-28; cross-team merge 2026-04-29)
Status: 8 tasks shipped, 5 superseded/dropped via cross-team merge with INITIATIVE_PRODUCT_TRUST_AND_IA Sprint 1, 8 still active. Foundation primitives in place; tab destinations + 2 sub-pages + modal-helper sweep complete.
...
- [ ] **Task F.0: Data availability audit** *(NEXT — BLOCKING T1.4)* — Read-only inspection of `products_core_table.dart` + `lib/services/score/` + detail-blob shape to determine whether 4-pillar scores exist as discrete fields, are derivable from `score_bonuses[]` / `score_penalties[]`, or need a pipeline ticket. Three-way verdict: GREEN / YELLOW (derive needed) / RED (pipeline blocker). Estimated 1–2 hours. Ping Sean with the verdict.
- [ ] **Task F.1: PGScoreRing reuse audit vs PGDonutChart** *(NEXT — BLOCKING T1.1)* — Decision-first: read `lib/core/widgets/pg_score_ring.dart` against the T1.1 score-led-hero contract (96pt center number with sub-label, verdict-tinted progress arc, tier-aware track color). If `PGScoreRing` covers it → reuse, no new primitive. Only build `PGDonutChart` if there is a concrete gap that can't be added to `PGScoreRing` without breaking existing call sites. Ping Sean with the decision.
- [ ] **Task F.2: PGPillarBar primitive** *(NEXT — BLOCKING T1.4)* — Build per spec (TDD: label + percent rendering, proportional bar fill, tone color from value tier, reduced-height variant). Ping Sean with the commit SHA on `main`.
- [ ] Task C.2: Switch / Checkbox / Slider adaptive sweep across `lib/` (parallel with F-chunk; no T1.x conflict).
- [ ] Task C.3: Motion-token sweep — replace ad-hoc `Duration(milliseconds:...)` and `Curves.*` references with `AppMotion.*` tokens (parallel).
...
- [ ] Task D.2: Empty-state audit — align bespoke screens with the home Recents empty-state pattern (parallel).
- [⏸] **Task B.3b: Frosted SliverAppBar + circular share + section audit** *(WAIT for T1.1 sign-off)* — Same file as T1.1 (`product_detail_screen.dart`); concurrent edits would cause merge churn. Hold until Trust/IA team signs off T1.1, then go mechanical (PGFrostedAppBar with empty title since hero carries product name, top-right `PGCircularIconButton(Icons.ios_share_rounded, haptic: false)` for share, optional `more_horiz_rounded` overflow deferred).
...
- [ ] Task E.1: Cross-screen smoke tests asserting `PGFrostedAppBar` (or `PGFrostedHeader` inside `PreferredSize` for non-sliver bodies like Profile Setup) is present on every migrated screen.
- [ ] Task E.2: Final `flutter analyze` + full-suite + push + this tracker close-out.

**Verification (in-flight, post-C.1):**
- `/Users/seancheick/Development/flutter/bin/flutter analyze` → No issues found
- `/Users/seancheick/Development/flutter/bin/flutter test` → 780/780 tests pass (up from 736 in Sprint 27.20 — +44 new tests across pg_frosted_app_bar, pg_circular_icon_button, pg_modal, plus the migrated screens' assertions still passing)
- 9 commits between `7c90b19` (PGFrostedAppBar) and `8339032` (PGModal sweep), all pushed to `origin/main`
