...
**Sprint 27.21: App-wide Apple-grade polish (in flight)** — 🟡 PARTIAL (2026-04-28)
Status: 8 of 16 plan tasks shipped + 1 deferred. Foundation primitives in place; tab destinations + 2 sub-pages + modal-helper sweep complete. Cross-cutting sweeps (C.2 controls, C.3 motion), refactor primitives (D.1), empty-state audit (D.2), smoke tests (E.1) and final tracker close (E.2) still queued. The Apple Altar product-detail hero (B.3a + B.3b) was deliberately deferred to a focused session — its scope (new safety-override `heroVerdictProvider` + 3 new hero widgets + 250-line `_HeaderSection` refactor + entrance choreography) needs simulator iteration that one-shot agent dispatches can't give. Plan: `docs/superpowers/plans/2026-04-28-app-wide-apple-grade.md`.
...
- [⏸] **Task B.3a + B.3b: Apple Altar product-detail hero — DEFERRED.** Scope (new `heroVerdictProvider` with safety-override priority ladder, three new hero widgets `_HeroVerdictBanner` + `_LimitedDataBanner` + `_HeroTrustChipOutline`, 250-line `_HeaderSection` refactor, staggered entrance choreography, `_ScoreRingButton.size` parameter, `_HeroMetaPill` → outline-chip migration, frosted app bar with top-right circular share button + section audit) is too large + too risky for one-shot agent dispatches without simulator iteration. Pulled out of this sprint into a dedicated follow-up sprint. The visual goal (96pt centered ring, full-width safety-aware verdict banner, separate Limited Data banner, single elevated PGCard) is unchanged.
...
- [ ] Task C.2: Switch / Checkbox / Slider adaptive sweep across `lib/`.
- [ ] Task C.3: Motion-token sweep — replace ad-hoc `Duration(milliseconds:...)` and `Curves.*` references with `AppMotion.*` tokens.
- [ ] Task D.1: Extract `PGAdaptiveBackButton` from `PGFrostedAppBar`'s inline back chevron (now redundant since Phase 0.2 introduced PGCircularIconButton — fold or skip).
- [ ] Task D.2: Empty-state audit — align bespoke screens with the home Recents empty-state pattern.
