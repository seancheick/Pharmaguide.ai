...
**Unified product-submission pipeline (2026-07-30; CODE COMPLETE, DEPLOYMENT PENDING):** Missing-product contributions and label-mismatch reports now use one typed, private submission spine with explicit consent, EXIF-stripped photos, no user narrative, owner-scoped RLS, immutable retry manifests, fail-closed status presentation, versioned AI drafts, allowlisted human approval, and proof-based catalog promotion. Approved labels enter the existing pipeline through one defensive `manual_label_v1` adapter and the canonical Clean → Enrich → Score → snapshot → cloud → Flutter release train; no second scorer or publication path exists. Private evidence has bounded, retry-safe retention, and permanent account deletion removes Storage before Auth and purges local stores only after confirmed server success. **Verified so far:** generated missing-product canary passed the real strict Clean → Enrich → Score pipeline; focused Flutter/service/security suites, Deno format/type checks, SQL parse, and adapter tests are green. **Still required before beta:** apply the migration, deploy the three Edge Functions and secrets, run authenticated RLS/Storage/reviewer/account-deletion canaries against the intended Supabase project, then release the app and pipeline.
